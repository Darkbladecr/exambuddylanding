...
if( ! function_exists('Recent_Projects_init') ) {
...
if( ! class_exists('Recent_Projects_Widget') ) {
	
	class Recent_Projects_Widget extends WP_Widget {

		function __construct() {
			$widget_ops = array('classname' => 'recent_projects_widget', 'description' => esc_html__( "The most recent projects on your site.",'salient-widgets'));
			parent::__construct('recent-projects', esc_html__('Recent Projects','salient-widgets'), $widget_ops);
		...
		function widget($args, $instance) {
			...
			$title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Projects','salient-widgets') : $instance['title']);
			...
			<div>
				...
				<a href="<?php echo esc_url( $the_project_link ); ?>" data-custom-grid-item="<?php echo esc_attr( $custom_content_project ); ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>">
					...
					if ( has_post_thumbnail() ) {
						...
						echo '<img src="'.get_template_directory_uri().'/img/no-portfolio-item-tiny.jpg" alt="'. esc_html__('no image added yet.','salient-widgets') . '" />';
					} ?>
	...
	?>
			<p><label for="<?php echo wp_kses_post( $this->get_field_id('title') ); ?>"><?php echo esc_html__('Title:', 'salient-widgets'); ?></label>
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id('number') ); ?>"><?php echo esc_html__('Number of projects to show:', 'salient-widgets'); ?></label>
			...
			<small><?php echo esc_html__('Change in increments of 3 (at most 9)', 'salient-widgets'); ?></small></p>
	<?php
		}
	}
}
