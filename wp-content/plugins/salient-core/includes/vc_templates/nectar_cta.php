...
if( $btn_style === 'material' ) {
  ...
  echo '<div class="nectar-cta '. esc_attr( $class ).'" data-style="'.esc_attr($btn_style).'" data-alignment="'.esc_attr($alignment).'" data-display="'. esc_attr($display) .'" data-text-color="'.esc_attr($text_color).'" '.$style_markup.'>';
...
// See through.
  ...
  echo '<div class="nectar-cta '.$class.'" data-color="'. esc_attr($button_color) .'" data-using-bg="'.esc_attr($using_bg_color).'" data-style="'.esc_attr($btn_style).'" data-display="'. esc_attr($display) .'" data-alignment="'.esc_attr($alignment).'" data-text-color="'.esc_attr($text_color).'" '.$style_markup.'>';
  echo '<'.esc_html($heading_tag). $style.'> <span class="text">'.wp_kses_post($text).' </span>';
  echo  '<span class="link_wrap" '.$style_padding_markup.'><a '.$target .' class="link_text" href="'.esc_url($url).'">'.wp_kses_post($link_text).'<span class="arrow"></span></a></span>'; 
  echo '</'.esc_html($heading_tag).'></div>';
} 
// Arrow Animation.
  ...
  echo  '<span class="link_wrap" '.$style_padding_markup.'><a '.$target .' class="link_text" href="'.esc_url($url).'"><span class="text">'.wp_kses_post($link_text) .'</span>'; 
...
// Next section link.
  ...
  if( $btn_type === 'down-arrow-bounce' ) {
    ...
    echo '<div class="nectar-next-section-wrap bounce" '.$style_markup.' data-custom-color="'.esc_attr($using_next_section_color).'"><a href="#" '.$next_section_color_style.' class="nectar-next-section skip-hash"> <i class="fa fa-angle-down"></i> </a></div>';
  } 
    ...
    echo '<div class="nectar-next-section-wrap down-arrow-bordered" '.$style_markup.' data-custom-color="'.esc_attr($using_next_section_color).'"><div class="inner" '.$next_section_color_style.'><a href="#" class="nectar-next-section skip-hash"><i class="fa fa-angle-down top"></i><i class="fa fa-angle-down"></i></a></div></div>';
  } 
    ...
    echo '<div class="nectar-next-section-wrap mouse-wheel" '.$style_markup.' data-custom-color="'.esc_attr($using_next_section_color).'"><a href="#" '.$next_section_color_style.' class="nectar-next-section skip-hash"><svg class="nectar-scroll-icon" viewBox="0 0 30 45" enable-background="new 0 0 30 45">
    ...
    echo '<div class="nectar-next-section-wrap minimal-arrow" '.$style_markup.' data-custom-color="'.esc_attr($using_next_section_color).'">
...
// All others.
  ...
  echo '<div class="nectar-cta '.esc_attr($class).'" data-color="'. esc_attr($button_color) .'" data-using-bg="'.esc_attr($using_bg_color).'" data-display="'. esc_attr($display) .'" data-style="'.esc_attr($btn_style).'" data-alignment="'.esc_attr($alignment).'" data-text-color="'.esc_attr($text_color).'" '.$style_markup.'>';
  echo '<'.esc_html($heading_tag). $style.'> <span class="text">'.wp_kses_post($text).' </span>';
  $border_color_attr = (!empty($underline_border_color)) ? 'style="border-color: '.esc_attr($underline_border_color).';"' : '';
  echo  '<span class="link_wrap" '.$style_padding_markup.'><a '.$target .' class="link_text" '.$border_color_attr.' href="'.esc_url($url).'">'.wp_kses_post($link_text).'</a></span>'; 
  echo '</'.esc_html($heading_tag).'></div>';
}
